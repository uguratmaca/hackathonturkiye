(function () {
    ...
    angular
        ...
        .component('hackathonList', {
            template: `
        <timeline>
            <timeline-event id="{{event.id}}" ng-if="$ctrl.route == 'old' ? event.date < $ctrl.today : (event.dateEnd || event.date) > $ctrl.today" ng-repeat="event in $ctrl.hackathons" side="{{$index%2===1?'right':'left'}}">
                ...
                when-not-visible="$ctrl.animateElementOut" >
                    ...
                    <timeline-footer>
                        <a class="btn btn-info" target="_blank" ng-href="{{event.url}}">Daha Fazla</a>
                  </timeline-footer>
                </timeline-panel>
            </timeline-event>
        </timeline>`,
